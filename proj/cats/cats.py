...
def pick(paragraphs, select, k):
    ...
    "*** YOUR CODE HERE ***"
    newParagraph = []
    for ch in paragraphs:
        if select(ch):
            newParagraph.append(ch)
    if k >= len(newParagraph):
        return ''
    else:
        return newParagraph[k]
    # END PROBLEM 1
...
def about(topic):
    ...
    # >>> about_dogs = about(['dog','dogs'])
    # >>> about_dogs("I'm a dog")
    DEBUG: ['im', 'a', 'dog']
    DEBUG: {'dog', 'dogs'}
    DEBUG: ['dog']
    ...
    def split_sentence(setence):
        return split(remove_punctuation(lower(setence)))
    
    def check(setence):
        """Return boolean on whether there's intersection with setence and topic
        
        """
        words = split_sentence(setence)
        
        # temp = list(set(words) & set(topic))
        # print(f"DEBUG: {words}")
        # print(f"DEBUG: {set(topic)}")
        # print(f"DEBUG: {temp}")
        if list(set(words) & set(topic)):
            return True
        else:
            return False
    return check
...
def accuracy(typed, reference):
    ...
    if len(typed_words) > len(reference_words):
        wrong_words += len(typed_words) - len(reference_words)
    
    len_num = min(len(typed_words), len(reference_words))
    for i in range(len_num):
        if typed_words[i] != reference_words[i]:
            wrong_words += 1

    # print(f"DEBUG: typed_words len = {typed_words}, reference len = {reference_words}, wrong words = {wrong_words}")
    return (1 - wrong_words / len(typed_words)) * 100
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    >>> ten_diff = lambda w1, w2, limit: 10 # Always returns 10 # w1 is typed_word, w2 is a word from word list
    ...
    for idx, word in enumerate(word_list):
        if word == typed_word:
            return typed_word
        if diff_function(typed_word, word, limit) < diff_min:
            index_of_min_word = idx
            diff_min = diff_function(typed_word, word, limit)
    if diff_min > limit:
        return typed_word
    else:
        return word_list[index_of_min_word]
