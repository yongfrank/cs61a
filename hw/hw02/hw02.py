from operator import add, index, mul
import re
...
def accumulate(merger, start, n, term):
    ...
    result = start
    while(index_of_n <= n):
        result = merger(result, term(index_of_n))
        index_of_n += 1
    return result
...
def summation_using_accumulate(n, term):
    ...
    55
    >>> summation_using_accumulate(5, triple) # 1*3 + 2*3 + 3*3 + 4*3 + 5*3
    45
...
def funception(func_a, start):
    ...
    "*** YOUR CODE HERE ***"
    def func_b(stop):
        if stop < 0 or start < 0: return None
        if start > stop: return func_a(start)

        index_of_func_b = start
        result = 1
        while(index_of_func_b < stop):
            result *= func_a(index_of_func_b)
            index_of_func_b += 1
